...
- Footer boilerplate (company info, social links, privacy/cookie policy links) is a built-in default, not something to rebuild per project — see README's "Footer defaults". `site/blueprints/site.yml` has a Footer tab (company fields, a `social` structure using the `icon` field type, and page pickers for `privacyPage`/`cookiePage`); `site/snippets/footer.php` renders it with empty-state fallbacks; `content/privacy-policy/` and `content/cookie-policy/` are unlisted placeholder pages to fill in per client. Social icons come from `tobimori/kirby-icon-field` (composer-installed into `site/plugins/kirby-icon-field/`, which is gitignored like `vendor/`/`kirby/` since it's composer-managed, not hand-written) reading SVGs from `assets/icons/` (tracked in git, unlike `assets/css`/`assets/js`) — add more platform SVGs there as needed.
